...
<head>
    <title>Rekap Transaksi - {{ $settings['nama_website'] ?? 'Bank Mini' }}</title>
    <style>
        body {
        ...
        .text-masuk { color: #16a34a; font-weight: bold; } /* Hijau */
        .text-keluar { color: #dc2626; font-weight: bold; } /* Merah */
        .text-id { color: #64748b; font-size: 8px; }
...
<body>
    ...
    <table class="table-data">
            ...
            @foreach ($transaksi as $item)
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <div style="font-size: 8px; color: #64748b;">{{ $item->no_rekening }}</div>
                    </td>
                    <td>
                        {{ $item->nasabah->kelas->nama_kelas ?? ($item->kelas_saat_transaksi ?? '-') }}<br>
                        <span style="font-size: 8px; color: #64748b;">
                            {{ $item->nasabah->kelas->jurusan->nama_jurusan ?? ($item->jurusan_saat_transaksi ?? '') }}
                        </span>
                    </td>
                    <td class="text-center">
                        <span class="font-bold">{{ $item->jenisTransaksi->nama_jenis ?? $item->kode_jenis }}</span>
                    </td>
                    <td style="font-style: italic; color: #475569;">{{ $item->keterangan_nasabah ?? '-' }}</td>
                    <td class="text-right {{ $isMasuk ? 'text-masuk' : 'text-keluar' }}">
                        {{ $isMasuk ? '+' : '-' }}Rp {{ number_format($nominal, 0, ',', '.') }}
                    </td>
                </tr>
            @endforeach
        </tbody>
        <tfoot>
            <tr style="background-color: #e2e8f0;">
                <td colspan="5" class="text-right font-bold">TOTAL MASUK (+)</td>
                <td class="text-right text-masuk">Rp {{ number_format($totalMasuk, 0, ',', '.') }}</td>
            </tr>
            <tr style="background-color: #e2e8f0;">
                <td colspan="5" class="text-right font-bold">TOTAL KELUAR (-)</td>
                <td class="text-right text-keluar">Rp {{ number_format($totalKeluar, 0, ',', '.') }}</td>
            </tr>
            <tr style="background-color: #cbd5e1;">
                <td colspan="5" class="text-right font-bold">SALDO NETTO REKAPITULASI</td>
    ...
    <table class="footer-sign">
        <tr>
            <td width="70%">
                <p style="font-size: 9px; color: #64748b;">Dicetak oleh: {{ $petugasLogin->nama }} ({{ $petugasLogin->kode }})</p>
            </td>
            <td class="text-center">
                Jakarta, {{ \Carbon\Carbon::now()->translatedFormat('d F Y') }}<br>
                Petugas Bank Mini,<br><br><br><br>
                <span class="nama-petugas">{{ $petugasLogin->nama }}</span>
            </td>
        </tr>
    </table>
