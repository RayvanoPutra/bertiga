...
@section('content')
    <div class="p-6">
        <div class="flex justify-between items-center mb-6">
            ...
            <button onclick="openModalKelas()"
                class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-bold shadow-lg transition-all flex items-center gap-2">
    ...
    <div id="modalKelas"
        class="fixed inset-0 bg-black/50 hidden z-50 flex items-center justify-center p-4 transition-all duration-300">
            ...
            <form id="formKelas" class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="input_nama_kelas" placeholder="Contoh: X RPL 1"
                            class="w-full pl-9 p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition font-semibold text-gray-800"
                            required>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="select_jurusan"
                            class="w-full p-3 border border-gray-300 rounded-xl bg-gray-50 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition font-medium"
                            required>
                            <option value="">Pilih Jurusan</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Tahun
                            Ajaran</label>
                        <select id="select_ta"
                            class="w-full p-3 border border-gray-300 rounded-xl bg-gray-50 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition font-medium"
                            required>
                ...
                <div id="errorKelas"
                    class="hidden p-3 bg-red-50 text-red-600 text-xs rounded-xl border border-red-100 font-bold flex items-center gap-2 italic">
...
@push('scripts')
    ...
    <script>
        ...
        // Mengambil data Jurusan & TA untuk select option
        ...
        async function fetchKelas() {
            ...
            tbody.innerHTML =
                '<tr><td colspan="6" class="text-center py-10 text-gray-400 italic">Memuat data...</td></tr>';
            ...
            try {
                ...
                if (fSearch) data = data.filter(i => i.nama_kelas.toLowerCase().includes(fSearch));

                tbody.innerHTML = '';
                if (data.length === 0) {
                    tbody.innerHTML =
                        '<tr><td colspan="6" class="text-center py-10 text-gray-400 italic">Data tidak ditemukan.</td></tr>';
                    return;
                }
                    ...
                    <tr class="hover:bg-blue-50/30 transition-colors border-b border-gray-100 last:border-0">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex justify-center gap-2">
                                <button onclick="editKelas('${item.kode_kelas}', '${item.nama_kelas}', '${item.kode_jurusan}', '${item.kode_tahun_ajaran}')" 
                                    class="p-1.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 active:scale-90 transition-all shadow-sm">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                    </svg>
                                </button>
                                <button onclick="deleteKelas('${item.kode_kelas}')" 
                                    class="p-1.5 bg-rose-500 text-white rounded-lg hover:bg-rose-600 active:scale-90 transition-all shadow-sm">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Berhasil!',
                        text: kodeLama ? 'Data kelas diperbarui' : 'Kelas baru telah ditambahkan',
                        timer: 1500,
                    ...
                    errorText.innerText = result.message || "Gagal menyimpan data";
        ...
        async function deleteKelas(kode) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    Swal.fire('Gagal!', result.message || "Data sedang digunakan.", 'error');
                }
            } catch (e) {
