...
@section('content')
    <div class="flex justify-between items-center mb-6 px-6 pt-6">
        ...
        <button onclick="openModalTA()"
            class="bg-blue-600 text-white px-5 py-2 rounded-lg font-bold hover:bg-blue-700 shadow flex items-center gap-2 transition transform hover:-translate-y-0.5">
    ...
    <div id="modalTA"
        class="modal opacity-0 pointer-events-none fixed w-full h-full top-0 left-0 flex items-center justify-center z-50 transition-all duration-300">
        <div class="modal-overlay absolute w-full h-full bg-black/50" onclick="closeModalTA()"></div>
        <div
            class="modal-container bg-white w-full max-w-md mx-auto rounded-xl shadow-2xl z-50 overflow-y-auto transform transition-all scale-95 opacity-0">
            <div class="py-4 text-left px-6">
                ...
                <form id="formTA" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="input_tahun_ajaran" placeholder="Contoh: 2024/2025"
                    ...
                    <div>
                        ...
                        <select id="input_status"
                            class="w-full p-3 border border-gray-300 rounded-lg outline-none bg-white font-semibold">
                            <option value="nonaktif">Tidak Aktif</option>
                            <option value="aktif">Aktif</option>
                        </select>
                    </div>
                    <div class="flex justify-end pt-4 space-x-2">
                        <button type="button" onclick="closeModalTA()"
                            class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg font-semibold">Batal</button>
                        <button type="submit" id="btnSimpanTA"
                            class="px-6 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition shadow-md">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        async function fetchTA() {
            ...
            try {
                ...
                if (data.length === 0) {
                    tbody.innerHTML =
                        '<tr><td colspan="4" class="text-center py-10 text-gray-400">Belum ada data tahun ajaran.</td></tr>';
                    return;
                }
        ...
        document.getElementById('formTA').addEventListener('submit', async (e) => {
            ...
            const url = kodeLama ? `${BASE_URL}/master/tahun-ajaran/${kodeLama}` :
                `${BASE_URL}/master/tahun-ajaran`;
