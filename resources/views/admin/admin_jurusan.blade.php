...
@section('content')
    <div class="p-6">
            ...
            <form id="filterForm" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div class="md:col-span-3">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="filterSearch" onkeyup="fetchJurusan()"
                            placeholder="Ketik nama atau kode jurusan..."
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                ...
                <button onclick="openModalJurusan()"
                    class="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-indigo-700 shadow-md flex items-center gap-2 transition transform active:scale-95">
    ...
    <div id="modalJurusan"
        class="fixed inset-0 bg-black/50 hidden z-50 flex items-center justify-center p-4 transition-all">
            ...
            <form id="formJurusan" class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <input type="text" id="input_kode_jurusan" placeholder="Contoh: TKJ"
                        class="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none font-mono font-bold uppercase"
                        required>
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="input_nama_jurusan" placeholder="Contoh: Teknik Komputer dan Jaringan"
                        class="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none font-semibold"
                        required>
                </div>

                <div id="errorJurusan"
                    class="hidden p-3 bg-red-50 text-red-600 text-xs rounded-xl border border-red-100 font-bold"></div>

                <div class="flex justify-end gap-3 pt-4 border-t">
                    <button type="button" onclick="closeModalJurusan()"
                        class="px-5 py-2.5 text-gray-500 font-bold hover:bg-gray-100 rounded-xl transition">Batal</button>
                    <button type="submit" id="btnSimpanJurusan"
                        class="px-8 py-2.5 bg-indigo-600 text-white font-bold rounded-xl shadow-lg hover:bg-indigo-700 transition transform active:scale-95">Simpan
                        Jurusan</button>
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        async function fetchJurusan() {
            ...
            try {
                ...
                if (fSearch) {
                    data = data.filter(i => i.nama_jurusan.toLowerCase().includes(fSearch) || i.kode_jurusan
                        .toLowerCase().includes(fSearch));
                }

                tbody.innerHTML = '';
                if (data.length === 0) {
                    tbody.innerHTML =
                        '<tr><td colspan="4" class="text-center py-10 text-gray-400">Data jurusan kosong.</td></tr>';
                    return;
                }
        ...
        document.getElementById('formJurusan').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (res.ok) {
                    alert("✅ Berhasil!\nJurusan ditambahkan dengan Kode: " + result.data.kode_jurusan);
                    closeModalJurusan();
                    fetchJurusan();
                } else {
